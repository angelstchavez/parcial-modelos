...
const SALARIOS_BASE: Record<Categoria, Record<Dedicacion, number>> = {
    ...
    'Asociado': {
        ...
        'Medio Tiempo': 1.990 * SALARIO_MINIMO_LEGAL_VIGENTE
    },
    'Titular': {
...
const BONIFICACION_POSGRADO: Record<Posgrado, number> = {
    'Especialización': 0.10 * SALARIO_MINIMO_LEGAL_VIGENTE,
    'Maestría': 0.45 * SALARIO_MINIMO_LEGAL_VIGENTE,
    'Doctorado': 0.90 * SALARIO_MINIMO_LEGAL_VIGENTE,
...
function calcularSalario(profesor: Profesor): number {
    ...
    let salarioBase = SALARIOS_BASE[profesor.categoria][profesor.dedicacion];

    // Aplicar bonificación por posgrado si existe
    let bonificacionPosgrado = profesor.posgrado ? BONIFICACION_POSGRADO[profesor.posgrado] : 0;

    // Aplicar bonificación por grupo de investigación si existe
    let bonificacionGrupoInvestigacion = profesor.grupoInvestigacion ? BONIFICACION_GRUPO_INVESTIGACION[profesor.grupoInvestigacion] : 0;

    // Calcular el salario total
    let salarioTotal = salarioBase + bonificacionPosgrado + bonificacionGrupoInvestigacion;
